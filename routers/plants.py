...
async def get_plant(plant_id: str, current_user: User = Depends(get_current_user)):
    if current_user.admin:
        try:
            ...
            plant.temperatures = sorted(plant.temperatures[:10], key=lambda l: l.timestamp, reverse=False)
            plant.humidities = sorted(plant.humidities[:10], key=lambda l: l.timestamp, reverse=False)
            plant.moistures = sorted(plant.moistures[:10], key=lambda l: l.timestamp, reverse=False)
            plant.light_values = sorted(plant.light_values[:10], key=lambda l: l.timestamp, reverse=False)
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Error: {str(e)}")
